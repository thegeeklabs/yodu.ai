...
class InfluxDb:
    ...
    def top_items_by_source(self, days_ago, action_type, tag_type, source, limit=5):
        ...
        :return:
        '''
        query = 'from(bucket: "bridgeml")\
        |> range(start: -' + str(days_ago) + 'd)\
        |> filter(fn: (r) => r["_measurement"] == "' + action_type + '")\
        |> filter(fn: (r) => r["' + tag_type + '"] == "' + source + '")\
        |> group(columns: ["item_id"])\
        |> aggregateWindow(every: ' + str(days_ago) + 'd, fn: sum, createEmpty: false)\
        |> group()\
